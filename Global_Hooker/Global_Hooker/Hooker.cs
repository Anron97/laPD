...
namespace Global_Hooker
{
    class Hooker
    {
        ...
        private void HookKeyPress(object sender, KeyEventArgs eventArgs)
        {
            ...
            if (eventArgs.KeyData == (Keys.PrintScreen))
            {
        ...
        {
            var dataInBuffer = Clipboard.GetDataObject();
            if (dataInBuffer.GetDataPresent(DataFormats.Bitmap))
            {
                Bitmap image = (Bitmap) dataInBuffer.GetData(DataFormats.Bitmap);
                image.Save("D:\\screenshots\\" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".bmp");
            }
            else
            {
                Console.WriteLine("No anything in buffer");
            }
        }
    }
}